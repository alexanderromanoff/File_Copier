...
class Application:
    ...
    def init_master(self):
        ...
        self.exitButton.grid(row=OVERDRIVE_ROW, column=3, sticky=EXPAND, padx=OFFSET*2)
    ...
    def copy(self):
        try:
            ...
            self.progressBar["maximum"] = self.copier.number_of_copies
    ...
    def schedule_copy(self):
        try:
            res = self.copier.copy_file(bool(self.use_ext), progress_callback=self.update_progress)
            if res:
                self.master.after(1, self.schedule_copy)
            else:
                self.clear()
        except Exception as ex:
            showerror(title="Error!", message=f"Unexpected error: {ex}, please contact the developer")
